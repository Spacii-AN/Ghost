...
const directCommandFiles = fs.readdirSync(commandsPath).filter(file => (file.endsWith('.js') || file.endsWith('.ts')) &&
 ...
 */
async function handleStopTrolling(userId, interaction) {
    try {
        removeTroll(userId);
